...
do
sshpass -p ${PWIBMC} ssh -o StrictHostKeyChecking=no -o ConnectTimeout=30 -t -t Administrator@${hostname} "ipmcget -t fru0 -d healthevents" >> ResultadoPsu.txt

chmod 700 ResultadoPsu.txt
cat ResultadoPsu.txt | egrep "PSU|Critical" >> ResultadoPsuFinal.txt
echo "Teste PSU OK" >> ResultadoPsuFinal.txt
cat ResultadoPsuFinal.txt
done
