...
export PWNOVA=$(oculta_senha "     - Entre com a NOVA senha:		 	 ")
export PWNOVA2=$(oculta_senha "     - Redigite a NOVA senha:			 ")
if [ "${PWNOVA}" != "${PWNOVA2}" ]
then
    echo " "
...
do
sshpass -p ${PWIBMC} ssh -o ConnectTimeout=30 -t -t ${USER}@${hostname} "ipmcset -t user -d privilege -v eSightMgmt 4" <<EOF > /tmp/output 2>&1
${PWIBMC}
EOF
cat tmp/output
done
